...
<h1>Instructor view of <a href="${course.url}">${course.courseName}</a>,
...
<table>
	...
	<c:forEach var="project" items="${projectList}" varStatus="counter">
		<tr class="r${counter.index % 2}">
			...
			<a href="${uploadProjectJarfileLink}"> active </a>
			</c:when>
			<c:when test="${project.initialBuildStatus == 'new'}">
			<a href="${uploadProjectJarfileLink}"> inactive </a>
			</c:when>
			</c:choose>
			</td>
...
<table>
  ...
  <tr>
    <td colspan="6">
    <c:url var="registerUWStudentsLink" 
      value="/view/instructor/registerUWStudents.jsp">
      <c:param name="coursePK" value="${course.coursePK}" />
    </c:url>
    <a href="${registerUWStudentsLink}">Register UW students for this
    course by uploading a classlist file</a></td>
  </tr>
<%--
	<tr>
...
--%>
	<tr>
		<td colspan=6><c:url var="registerOneStudentLink"
			...
			<c:param name="coursePK" value="${course.coursePK}" />
		</c:url> <a href="${registerOneStudentLink}"> Register one student for this
	...
	<tr>
		<td colspan=6>
			...
			<a href="${registerInstructorLink}"> Register an Instructor or TA for this course 
