...
public class PerformLogin extends SubmitServerServlet {
	...
	String campusUID = request.getParameter("campusUID");
		...
		try {
			/* ON: Hack check */
			// if (!superUserLogin && !campusUID.equals(request.getRemoteUser())) {
			if (!superUserLogin && (campusUID.indexOf(request.getRemoteUser()) == -1)) {
	            		throw new ClientRequestException("Supplied userid doesn't match remote user: " +request.getRemoteUser());
			}

			getAuthenticationLog().info("Logging in remote_user '" + request.getRemoteUser() + "' as '" + campusUID + "'");
			if (!campusUID.equals(request.getRemoteUser())) {
			getAuthenticationLog().info("remote_user doesn't match campusUID !");
			}
