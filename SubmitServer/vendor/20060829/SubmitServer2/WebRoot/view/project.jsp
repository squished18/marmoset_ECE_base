...
<table>
	...
	<c:forEach var="submission" items="${submissionList}"
		...
		<td>
			<c:if test="${backgroundRetestMap[submission.submissionPK].numFailedBackgroundRetests > 0}">
		...
		<c:when test="${submission.buildStatus == 'complete'}">
			<td colspan="${projectJarfile.valueReleaseTests > 0 ? 4 : 2}">did
				not compile</td>
			<td><a href="${submissionLink}">view</a></td>
		</c:when>
		<c:otherwise>
			<td colspan="${projectJarfile.valueReleaseTests > 0 ? 5 : 3}">not
			tested yet</td>
		</c:otherwise>
		</c:choose>
